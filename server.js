...
const MongoClient = require('mongodb').MongoClient  //makes it possible to use methods associated with MongoClient and talk to ur DB
...
let db, //declare db as a global varibale called to db but not assign a value
    dbConnectionStr = process.env.DB_STRING, //declaring a variable and assigning our database connection string to it
    dbName = 'todo'  //declaring a variable and assign the name of the database we will be using
...
app.use(express.urlencoded({ extended: true })) //Tells express to decode and encode URLs where the header mathces the content. Supports arrays and objects
...
app.post('/addTodo', (request, response) => { //starts a POST method when the add route is passed in
    ...
    .then(result => {   //if insert i successful, do something
        console.log('Todo Added') //console log action
        response.redirect('/') //gets rid if the /addTodo route, and redirects back to the homepage
    })  //closing the .then 
    .catch(error => console.error(error))   //catcing errors
})  //ending the POST
...
app.put('/markUnComplete', (request, response) => { //starts a PUT method when the markUnComplete route is passed in
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{  //look in the db for one item matching the name of the item passed in the main.js file that was clicked on
        $set: { 
